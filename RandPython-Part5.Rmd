...
In these techniques an ensemble (collection of trees) is created and then these trees are 
averaged (random forest) or a trees is created and its performance improved incrementally
(boosting).  These techniques use a methof known as 'bootstrapping'. In bootstraping when
we have a data set of size 'N' then we create a bootstrap sample of size 'N' by sampling the
original dataset with replacement. 

In the Random Forest we create 'N' number of random trees from different bootstrap samples
and also at each split a random subset of feature variables is chosen. Finally all the trees
are averaged which tends to average out the overall error in the individual trees

Gradient Boosted Decision Trees also use an ensemble of trees but they don't build trees with random set of features at each step. Rather small and simple trees are built and then the successive trees try to minimize the error from the earlier trees.
