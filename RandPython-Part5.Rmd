...
There aren't as much treatment of splines in Python. I did find the LSQUnivariate spline. This requires the explcit setting of knots
```{python}
import numpy as np
import pandas as pd
import os
...
in which we use a  different functions for each of the variables. GAMs give a much better fit since we can choose any function for the different sections
...
## 1.2b ## 1.2 a Generalized Additive Models (GAMs) - Python Code
...
given a sample of size N, we create datasets of size N by sampling this original dataset with replacement. Machine Learning models are built on the different samples and then averaged.
...
Bagging, or  Bootstrap Aggregation decreases the variance of prediction by creating
separate Decisiion Tree based ML models on the different samples and then averaging
these ML models


Bagging is a greedy algorithm and tries to produce splits based on all variables which try to minimize the error. However the different ML models have a high correlation. Random Forests remove this shortcoming, by using a variable and random set of features to split on. Hence the features chosen and the resulting trees are uncorrelated. When these ML models are averaged the performance is much better.

Gradient Boosted Decision Trees also use an ensemble of trees but they don't build Machine Learning models  with random set of features at each step. Rather small and simple trees are built. Successive trees try to minimize the error from the earlier trees.


Out of Bag (OOOB) Error - In Random Forest and Gradient Boosting for each bootstrap sample taken from the dataset, there will be samples left out. These are known as Out of Bag samples.Classification accuracy carried out on these OOB samples is known as OOB error
